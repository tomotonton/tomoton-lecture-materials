...
<head>
  ...
  <style>
    ...
    /* 未選択時のウェルカム画面（左上・控えめ表示） */
    .welcome { flex: 1; display: block; padding: 16px 20px; overflow: auto; color: #333; }
    body.hasPage .welcome { display: none; }
    .welcomeInner { max-width: 600px; }
    .welcomeQuote { font-size: 15px; font-weight: 500; line-height: 1.8; color: #555; margin: 0 0 14px; }
    .welcomeHelp { color: var(--muted); font-size: 13px; line-height: 1.8; }
    .welcomeOpenHint { display: none; background: #f5f8fd; border: 1px solid #dde7f4; border-radius: 8px; padding: 8px 12px; margin: 0 0 10px; color: #3a6ea5; font-size: 13px; }
...
<body>
  ...
  <script>
    (function () {
      ...
      // 未選択時のウェルカム画面：ひとことをランダム表示（控えめ）
      const QUOTES = [
        "継続は間違いなく力です！",
        "身に着けた力は、誰にも奪われることがない皆さん自身の財産です！",
        "間違った箇所は、あなたの宝物。大切に消化して。",
        "動いた！の感動。それはきっとエンジニアの原動力。",
        "バグは、まだ見ぬ正解への道しるべ。",
        "エラーメッセージは、コンピュータからの手紙。",
        "コードは、考えた通りに正直に動く。",
        "小さく作って、動かして、また直す。",
        "「動いた」の次は「なぜ動いた」。",
        "詰まったら、小さく分けて試す。",
