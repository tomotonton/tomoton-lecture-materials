...
(function () {
  ...
  function friendlyError(raw, code) {
    ...
    if (isTimeout) {
      ...
      if (!nm || /^(printf|scanf|puts|putchar|getchar)$/.test(nm)) {
        hints.push("<code>printf</code>/<code>scanf</code> を使うには、先頭に <code>#include &lt;stdio.h&gt;</code> が必要です。");
      }
      if (nm && /^(strlen|strcpy|strcmp|strcat)$/.test(nm)) {
        hints.push("文字列を扱う関数を使うには <code>#include &lt;string.h&gt;</code> が必要です。");
      }
    } else if (isParse) {
  ...
  function build() {
    ...
    btnCompile.onclick = function () {
      ...
      // scanf 等の「入力切れ」は実行時エラーであって構文の問題ではないため、
